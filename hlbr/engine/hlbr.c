...
#ifdef KEEP_LOGFILE_OPEN
/**
 * Opens a log file for use, and insert an entry in Globals.LogFiles[].
 * If there is already 
 */
LogFileRec* OpenLogFile(char* name)
{
	...
	for (i=0; i<Globals.NumLogFiles; i++) {
	...
	if (f == NULL && Globals.NumLogFiles < MAX_LOG_FILES) {
		// allocate new file, update f
		strncpy(Globals.LogFiles[Globals.NumLogFiles]->fname, name, 1024-1);
		Globals.LogFiles[Globals.NumLogFiles]->fp = fopen(name, "a");
		if (!Globals.LogFiles[Globals.NumLogFiles]->fp) {
			fprintf(stderr, "Error opening log file: %s\n", name);
			hlbr_mutex_unlock(&LogThreadMutex);
			return f;
		}
		Globals.LogFiles[Globals.NumLogFiles]->FileMutex = 0;
		Globals.LogFiles[Globals.NumLogFiles]->FileLockID = 0;
		f = &Globals.LogFiles[Globals.NumLogFiles++];
	}
...
#else //KEEP_LOGFILE_OPEN
LogFileRec* OpenLogFile(char* name)
{
	LogFileRec* f;

	f = MALLOC(sizeof(LogFileRec));
	strncpy(f->fname, name, 1024-1);
	f->fp = fopen(name, "a");
	if (!f->fp) {
		fprintf(stderr, "Couldn't open log file \"%s\" for appending\n", name);
		free(f);
		return NULL;
	}
